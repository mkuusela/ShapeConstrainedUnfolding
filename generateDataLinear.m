...
% Store f
fFun = @(s) 4/3*1e4*lumFactor/(ubE-lbE)^2*(ubE-s); % Scaled so that lumFactor has the same meaning as with inc. jets
fGridE = fFun(gridE);
...
fPertFun = @(s) -2*lumFactor/(ubE-lbE)^2*(s-ubE);
...
save(['./data/linearData_lumFactor',num2str(lumFactor),'nBinsE',num2str(nBinsE),'nBinsF',num2str(nBinsF),'.mat'],'fGridE','fPertGridE','gGridF','fBinsE','fPertBinsE','KSpline','KHistTrue','KHistPert','KSpline_noSmear','mu','y','ySeveralSamples','knots','gridE','gridF','binsE','binWidthsE','binsF','p','nGridE','nGridF','nBinsE','nBinsF','order','lbE','ubE','lbF','ubF');